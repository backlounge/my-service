...
public/                      ← 公開されるファイル一式(Cloudflare Pagesの公開フォルダ)
  ...
  admin/
    ...
    project-detail.html           ← 案件詳細画面(編集・ステータス変更・ファイル添付)
  404.html
  robots.txt
  sitemap.xml
  ...
  js/admin-project-detail.js ← 案件詳細の編集・削除・添付ファイル管理
...
schema.sql                  ← D1のテーブル定義(contacts, users, login_attempts, site_settings, files, projects)
